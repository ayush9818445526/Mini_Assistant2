...
class DualHandGestureController:
    """
    Fixed dual-hand gesture controller with improved clicking and thumb detection
    """
    ...
    def __init__(self, screen_controller=None):
        ...
        self.gesture_cooldown = 1.0  # Increased to reduce spam clicking
        self.frame_skip = 3  # Increased to reduce processing
        self.frame_count = 0
        ...
        self.cursor_hold_time = 0.3  # Slightly longer hold time
        
        # Click detection in cursor mode
        self.cursor_click_cooldown = 1.5  # Separate cooldown for cursor clicks
        self.last_cursor_click_time = 0
        ...
        print("🤲 Dual-Hand Gesture Controller (Fixed) initialized")
        print("💡 Show both hands (8-10 fingers) for cursor mode")
        print("💡 In cursor mode: Join 2 fingers to click")
    ...
    def enable(self):
        ...
        print("🖐️🖐️ Both hands (8-10 fingers): Cursor control + 2-finger click")
        return True
    ...
    def _capture_loop(self):
        ...
        while self.is_running:
            try:
                ...
                time.sleep(0.05)  # Slightly slower processing
    ...
    def _handle_cursor_mode(self, all_hands, total_fingers):
        ...
        if not self.cursor_active:
            ...
            if hold_duration >= self.cursor_hold_time:
                ...
                print("👆👆 Join 2 fingers together to click")
        
        if self.cursor_active:
            # Check for 2-finger click gesture in cursor mode
            self._check_cursor_click(all_hands)
            
            # Update cursor position using center of all hands
            self._update_cursor_from_hands(all_hands)
    
    def _check_cursor_click(self, all_hands):
        """Check for 2-finger click gesture in cursor mode"""
        try:
            current_time = time.time()
            
            # Avoid spam clicking
            if current_time - self.last_cursor_click_time < self.cursor_click_cooldown:
                return
            
            # Look for 2 fingers close together (click gesture)
            for hand_landmarks in all_hands:
                fingers_up = self._get_fingers_up(hand_landmarks)
                total_fingers_this_hand = sum(fingers_up)
                
                # Check for exactly 2 fingers (index + middle or thumb + index)
                if total_fingers_this_hand == 2:
                    # Check if it's index + middle (peace sign style)
                    if fingers_up == [0, 1, 1, 0, 0]:  # Index + Middle
                        # Check if fingers are close together (pinch gesture)
                        ...
                        if distance < 0.05:  # Fingers close together
                            print("[Debug] Cursor click - 2 fingers joined!")
                            self._perform_cursor_click()
                            self.last_cursor_click_time = current_time
                            return
                    ...
                    # Check for thumb + index pinch
                        ...
                        if distance < 0.05:  # Fingers close together
                            print("[Debug] Cursor click - thumb + index pinch!")
                            self._perform_cursor_click()
                            self.last_cursor_click_time = current_time
                            return
                            
        except Exception as e:
            print(f"❌ Error checking cursor click: {e}")
    
    def _perform_cursor_click(self):
        """Perform click at current cursor position"""
        try:
            import pyautogui
            if self.last_cursor_x and self.last_cursor_y:
                pyautogui.click(self.last_cursor_x, self.last_cursor_y)
                print(f"[CursorClick] Clicked at ({self.last_cursor_x}, {self.last_cursor_y})")
            else:
                # Fallback to center click
                screen_width, screen_height = pyautogui.size()
                pyautogui.click(screen_width // 2, screen_height // 2)
                print("[CursorClick] Clicked at center")
        except Exception as e:
            print(f"❌ Error performing cursor click: {e}")
    ...
    def _detect_single_hand_gesture(self, hand_landmarks):
        """Detect gestures with single hand (improved thumb detection)"""
        try:
            fingers_up = self._get_fingers_up(hand_landmarks)
            total_fingers = sum(fingers_up)
            ...
            if total_fingers >= 4:
                gesture = "scroll_down"
                if self.frame_count % 10 == 0:  # Less frequent debug
            ...
            # Peace sign - index + middle (but not too close together)
            elif fingers_up == [0, 1, 1, 0, 0]:
                # Check if fingers are NOT too close (to avoid cursor click confusion)
                index_tip = hand_landmarks[8]
                middle_tip = hand_landmarks[12]
                distance = math.sqrt((index_tip[0] - middle_tip[0])**2 + (index_tip[1] - middle_tip[1])**2)
                
                if distance > 0.05:  # Fingers spread apart (normal peace sign)
                    gesture = "click"
                    print(f"[Debug] Single hand - Peace sign")
            
            # Improved Thumbs up detection
            elif fingers_up[0] == 1 and sum(fingers_up[1:]) == 0:  # Only thumb up
                # Check if thumb is pointing up (y coordinate smaller than wrist)
                thumb_tip = hand_landmarks[4]
                wrist = hand_landmarks[0]
                thumb_mcp = hand_landmarks[2]  # Thumb base joint
                
                # Check thumb orientation - tip should be above base
                if thumb_tip[1] < thumb_mcp[1] - 0.05:  # More lenient threshold
                    gesture = "play_pause"
                    print(f"[Debug] Single hand - Thumbs up (improved detection)")
    ...
    def _get_fingers_up(self, landmarks):
        """Determine which fingers are extended for one hand (improved)"""
        fingers = []
        ...
        thumb_extended = thumb_tip[0] > thumb_ip[0] if thumb_tip[0] > landmarks[0][0] else thumb_tip[0] < thumb_ip[0]
    ...
    def _handle_gesture(self, gesture):
        ...
        try:
            ...
            if gesture == "scroll_down":
                ...
                import pyautogui
                # Always click at center for single-hand gestures
                screen_width, screen_height = pyautogui.size()
                pyautogui.click(screen_width // 2, screen_height // 2)
                ...
                print("[DualHandGesture] Play/Pause (spacebar)")
